...
$.post("helper/master/quick-enquiry",{"operation":"viewmore","id":button.data("id")},function(data){
$(".enqfull").html(data);
});
});
...
function StatusUpdate(id,type){
  var url = "helper/master/quick-enquiry";
  $.ajax({
    type:"POST",
    url:url,
    dataType:"json",
...
function fetchData(){$.post("helper/master/quick-enquiry",{"operation":"fetch"},function(data){$(".loadpan").html(data);$('#entry_table').DataTable({stateSave:true});});}
</script>
</body>
</html>